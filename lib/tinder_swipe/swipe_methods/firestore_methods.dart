  ...
  Future<String> createGroup(
    ...
    try {
      GroupActivityPackage groupActivity = GroupActivityPackage(
        ...
        activityCounter: chosenActivities, // what activities have been liked?
      );
...
// needed to get Activity -> has voted / chosesn activities
  ...
  Future addSecondRoundVoting(String activityUid, List<dynamic> activities,
      List<dynamic> userThatVotedInSecondRound, String finalActivity) async {
    await _firestore
        .collection("groups")
        .doc(activityUid)
        .update({"secondRoundActivities": activities});

    await _firestore
        .collection("groups")
        .doc(activityUid)
        .update({"hasVotedInSecondRound": userThatVotedInSecondRound});

    await _firestore
        .collection("groups")
        .doc(activityUid)
        .update({"secondRoundMainActivity": finalActivity});
  }
}
